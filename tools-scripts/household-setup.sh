...
echo ""
echo "--- Summons card (6 lines, for wallet + phone lock screen) ---"
echo "Leave blank to skip any line; you can fill in later with pen."
ICE_NAME=$(ask "ICE (in-case-of-emergency) contact name" "")
ICE_PHONE=$(ask "ICE phone" "")
MEET_ADDR=$(ask "Primary meeting point (street address)" "")
MY_NAME=$(ask "Your name (for the card)" "")
MY_BLOOD=$(ask "Your blood type" "")
MY_ALLERGY=$(ask "Your #1 allergy (or 'none')" "none")
...
DEFAULT_OUT="$PWD/shtf-personal-checklist.md"
echo ""
echo "Checklist and summons card will be saved to the current directory:"
echo "  $PWD"
echo ""
...
# Normalize blanks so the card still prints the line labels.
...
[ -z "$MY_BLOOD" ] && MY_BLOOD="___"
[ -z "$MY_ALLERGY" ] && MY_ALLERGY="__________"

{
    ...
    echo "MEET: $MEET_ADDR"
    echo "ME: $MY_NAME  Blood: $MY_BLOOD  Allergy: $MY_ALLERGY"
    echo "BLEEDING -> Limb spurting? TOURNIQUET high & tight. Else press hard."
    echo "WATER (clear) -> 8 drops unscented bleach per gallon, wait 30 min."
    echo "WHAT NOW? -> default STAY. Leave only if home is unsafe."
    echo "-----------------------------------------------"
    echo "Print 6-up, laminate, carry one. Screenshot to lock screen."
} > "$SUMMONS_OUT"
...
echo "  4. Print $SUMMONS_OUT 6-up and laminate (FedEx/UPS ~\$2/card)"
echo "     Screenshot it to your phone lock screen."
echo ""
echo "----- 30-SECOND CONFIDENCE DRILL -----"
echo "Do this now. From where you are sitting, without getting up:"
echo "  [ ] Name two exits from your bedroom."
echo "  [ ] Name the closest stop-the-bleed card location."
echo "  [ ] Point to your water shutoff."
echo "Hesitated on any? Fix that today, not someday."
echo "---------------------------------------"
echo ""
echo "Do not commit your personal checklist or summons card to git."
